    ...
    @objc func submitTapped(_ sender: UIButton) {
        ...
        if let solutionPosition = solutions.firstIndex(of: answer) {
            ...
            if score % 7 == 0 {
                ...
                alert.addAction(UIAlertAction(title: "Let's go!", style: .default, handler: { [weak self] _ in self?.levelUp()}))
                present(alert, animated: true)
            }
        } else {
            let ac = UIAlertController(title: "Wrong", message: "You guessed incorrectly", preferredStyle: .alert)
